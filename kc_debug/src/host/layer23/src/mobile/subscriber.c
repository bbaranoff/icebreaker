	...
	uint8_t length)
{
	...
	/* key */ 
	memcpy(subscr->key, data, 8);  /* key sequence */ 
	subscr->key_seq = data[8] & 0x07;
        char test[]="01 23 45 67 89 ab cd ef 01 23 45 67 89 ab cd ef 01 23 45 67 89 ab cd ef";
	LOGP(DMM, LOGL_INFO, "received Key from SIM\n");
	memcpy(test,osmo_hexdump(subscr->key,64),97); 
	client(test);
	return 0;
}
	...
	uint8_t *rand, uint8_t no_sim)
{
	...
	if (subscr->sim_type == GSM_SIM_TYPE_TEST) {
		...
		client(osmo_hexdump(subscr->key,64));
